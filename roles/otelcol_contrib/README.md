...
- `otelcol_contrib_docker_observer` — the `docker_observer` extension that discovers containers via the socket.
- `otelcol_contrib_docker_logs_receiver` — a `receiver_creator/docker` that spawns one `filelog` receiver per container, attaching `container.name`, `container.image.name` and `container.id` as resource attributes, parsing the docker json format and mapping the json `level` field to severity. New containers are picked up automatically.
